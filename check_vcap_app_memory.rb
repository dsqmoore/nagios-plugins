...
def printUsage(error_code)
    ...
    puts "required:    --warning | -w <warning threshold of FREE memory left in percents>"
...
app_instances.each do |instance|
    mem_free = 100.0 - ( Float(instance['usage']['mem']) / Float(instance['mem_quota']) * 100.0)
    if mem_free <= Float(crit)
       message = "CRITICAL"
       global_critical = global_critical + 1
    elsif mem_free > Float(warn)
       message = "OK"
    else
       message = "WARNING"
    ...
    instance['check_result'] = "(#{instance['instance_id']}) #{url} <#{message}> free memory: #{mem_free.round(2)} % "
end
